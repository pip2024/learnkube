...
Each example subdirectory (`secrets/`, `security/`, `stateful/`, `argocd/`, `operator/`, `otel/`, `scaling/`) is self-contained with its own manifests and its own `README.md` walkthrough — read that file rather than guessing, each has its own prerequisites and cleanup steps.
...
- `scaling/` — three layers of "scaling," only two of which actually run: `HorizontalPodAutoscaler` (pod count) and in-place Pod vertical resize (`kubectl patch --subresource=resize`, K8s 1.33+/stable 1.35, pod size — the built-in mechanism, chosen over installing the third-party VPA project). Karpenter (node count) is covered conceptually only in the README — it fundamentally can't run against minikube (no cloud API to provision nodes from), not just "skipped for simplicity" like VPA.
