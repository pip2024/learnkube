...
Each example subdirectory (`secrets/`, `security/`, `stateful/`, `argocd/`, `operator/`) is self-contained with its own manifests and its own `README.md` walkthrough — read that file rather than guessing, each has its own prerequisites and cleanup steps.
...
**The shared app** (`app/server.py`, image `learnkube:v1`, built from `app/Dockerfile`) is a small Flask server whose behavior is driven entirely by three file/env inputs, not application logic — this is deliberate, so different examples can exercise different Kubernetes mechanisms against the exact same image:
